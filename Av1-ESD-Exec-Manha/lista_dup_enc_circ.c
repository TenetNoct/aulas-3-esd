...
bool lista_dup_enc_circ_retira_sublista(ListaDupEncCirc* l1, const ListaDupEncCirc* l2) {
    if (!l1 || !l2 || lista_dup_enc_circ_vazia(l1) || lista_dup_enc_circ_vazia(l2) || l2->tamanho > l1->tamanho) {
        return false; // Condições inválidas ou impossíveis
    }

    NoListaDupEncCirc* inicio_l1 = l1->cabeca->proximo;
    NoListaDupEncCirc* no_l1 = inicio_l1;
    int tamanho_l2 = l2->tamanho;

    // Percorre l1 procurando o início da sequência de l2
    do {
        NoListaDupEncCirc* atual_l1 = no_l1;
        NoListaDupEncCirc* atual_l2 = l2->cabeca->proximo;
        bool achou_sublista = true;
        int count = 0;

        // Compara a sequência a partir do nó atual de l1 com l2
        while (atual_l2 != l2->cabeca && atual_l1 != l1->cabeca) {
            if (atual_l1->dado != atual_l2->dado) {
                achou_sublista = false;
                break;
            }
        ...
        // Verifica se a sublista completa foi encontrada
        if (achou_sublista && count == tamanho_l2) {
            // Encontrou a sublista, agora remove os nós
            NoListaDupEncCirc* no_remover = no_l1;
            NoListaDupEncCirc* anterior_sub = no_l1->anterior;
            NoListaDupEncCirc* proximo_sub = atual_l1; // Nó após a sublista

            for (int i = 0; i < tamanho_l2; i++) {
                NoListaDupEncCirc* proximo_remover = no_remover->proximo;
                free(no_remover);
                no_remover = proximo_remover;
            }

            // Reconecta a lista l1
            anterior_sub->proximo = proximo_sub;
            proximo_sub->anterior = anterior_sub;
            l1->tamanho -= tamanho_l2;

            return true; // Sublista removida com sucesso
        }

        // Avança para o próximo nó em l1
        no_l1 = no_l1->proximo;

    } while (no_l1 != inicio_l1); // Continua até percorrer toda l1

    return false; // Sublista não encontrada
}