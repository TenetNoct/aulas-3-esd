...
PilhaSeq* pilha_seq_criar(int capacidade) {
    ...
    p->topo_idx = -1; // Indica pilha vazia
    return p;
}
...
bool pilha_seq_vazia(const PilhaSeq* p) {
    if (!p) return true; // Considera nulo como vazio
    return p->topo_idx == -1;
}

bool pilha_seq_cheia(const PilhaSeq* p) {
    if (!p) return false; // Nulo não está cheio
...
bool pilha_seq_desempilhar(PilhaSeq* p, TipoElementoPilha* elemento) {
    ...
    if (elemento) { // Só copia se o ponteiro for válido
